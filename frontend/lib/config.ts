...
export const config: AppConfig = {
  ...
  features: {
    enableDebugMode: getEnvironment() === 'development' || process.env.NEXT_PUBLIC_DEBUG_MODE === 'true',
...
export const getFullApiUrl = (path: string): string => {
  ...
  if (config.features.enableDebugMode) {
    console.log(`[getFullApiUrl] Input path: "${path}" -> Clean path: "${cleanPath}" -> Full URL: "${fullUrl}"`);
  }
